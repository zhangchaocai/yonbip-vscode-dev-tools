...
export class ProjectContextCommands {
    ...
    private static async handleInitProjectContext(
        ...
        try {
            ...
            // 确认操作 - 使用自定义弹窗
            const confirm = await this.showCustomConfirmationDialog(
                '初始化项目',
            ...
            const iconUpdated = await IconThemeUpdater.addModuleToIconTheme(moduleName);
...
<module name="${dirName}">
     ...
     * @returns 用户是否确认
     */
    private static async showCustomConfirmationDialog(title: string, message: string): Promise<boolean> {
        return await CustomDialogUtils.showCustomConfirmationDialog(title, message);
    }
