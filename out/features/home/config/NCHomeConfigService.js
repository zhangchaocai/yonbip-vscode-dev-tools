...
const PropXmlUpdater_1 = require("../../../shared/utils/PropXmlUpdater");
...
class NCHomeConfigService {
        ...
        if (configCopy.dataSources && configCopy.dataSources.length > 0) {
            for (const dataSource of configCopy.dataSources) {
                if (dataSource.password) {
                    const decryptedPassword = PasswordEncryptor_1.PasswordEncryptor.getSecurePassword(configCopy.homePath, dataSource.password);
    ...
    async saveConfig(config) {
        try {
            ...
            if (this.config.vmParameters !== undefined && this.config.homePath) {
                try {
                    PropXmlUpdater_1.PropXmlUpdater.updateVmParametersInPropXml(this.config.homePath, this.config.vmParameters);
    ...
    async testConnection(dataSource) {
        try {
            ...
            const securePassword = PasswordEncryptor_1.PasswordEncryptor.getSecurePassword(this.config.homePath, dataSource.password || '');
...
- 查看监听器日志`;
    ...
    async addDataSource(dataSource) {
        ...
        if (this.config.homePath) {
            try {
                PropXmlUpdater_1.PropXmlUpdater.updateDataSourceInPropXml(this.config.homePath, dataSource, false);
    ...
    async updateDataSource(dataSource) {
        ...
        if (this.config.homePath) {
            try {
                PropXmlUpdater_1.PropXmlUpdater.updateDataSourceInPropXml(this.config.homePath, dataSource, true);
    ...
    async setAsDesignDatabase(dataSourceName) {
        ...
        if (this.config.homePath) {
            try {
                PropXmlUpdater_1.PropXmlUpdater.removeDataSourceFromPropXml(this.config.homePath, originalDataSourceName);
                PropXmlUpdater_1.PropXmlUpdater.updateDataSourceInPropXml(this.config.homePath, dataSource, false);
                ...
                try {
                    ...
                    PropXmlUpdater_1.PropXmlUpdater.updateDataSourceInPropXml(this.config.homePath, originalDataSource, false);
                }
                catch (restoreError) {
