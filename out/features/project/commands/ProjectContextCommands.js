...
const HomeService_1 = require("../../home/HomeService");
const ClasspathService_1 = require("../../library/service/ClasspathService");
...
class ProjectContextCommands {
    ...
    static async handleInitProjectContext(uri, libraryService, configService) {
        try {
            ...
            const confirm = await vscode.window.showWarningMessage(`将在以下目录初始化项目：${selectedPath}\n\n这将创建build/classes目录并初始化Java项目库。是否继续？`, '继续', '取消');
            if (confirm !== '继续') {
                return;
            }
            ...
            const startService = await vscode.window.showInformationMessage(`项目初始化完成！是否立即在目录 ${selectedPath} 中启动HOME服务？`, '启动', '取消');
            if (startService === '启动') {
                if (extensionContext) {
                    const homeService = new HomeService_1.HomeService(extensionContext, configService);
