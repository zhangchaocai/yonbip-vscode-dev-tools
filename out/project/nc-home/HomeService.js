...
const ToolbarIconService_1 = require("./ToolbarIconService");
class HomeService {
    context;
    configService;
    process = null;
    ...
    async startHomeService(selectedPath) {
        ...
        try {
            ...
            const selectedServiceDirectory = ServiceStateManager_1.ServiceStateManager.getSelectedServiceDirectory();
            ...
            if (this.process) {
                this.process.stdout?.on('data', (data) => {
                    ...
                    if (output.includes('Server startup in') ||
                        ...
                        StatisticsService_1.StatisticsService.incrementCount(StatisticsService_1.StatisticsService.HOME_START_COUNT);
                    }
                });
            }
            if (this.process) {
    ...
    buildClasspath(config, coreJarPath, workspaceFolder) {
        ...
        const libDirs = [
            ...
            path.join(config.homePath, 'webapps'),
        ...
        if (classpathString.length > 7000) {
    ...
    setStatus(status) {
        ...
        const toolbarIconService = ToolbarIconService_1.ToolbarIconService.getInstance(this.context);
...
<DataSourceMeta>
    ...
    updateStatusBarModuleInfo(moduleName, modulePath) {
        if (this.statusBarItem) {
    ...
    updateStatusBarDisplay(message) {
        if (this.statusBarItem) {
