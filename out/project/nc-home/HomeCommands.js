...
class HomeCommands {
    ...
    async startHomeService(selectedPath) {
        try {
            ...
            if (selectedPath) {
                try {
                    await ServiceStateManager_1.ServiceStateManager.saveSelectedServiceDirectory(selectedPath);
                }
                catch (saveError) {
    ...
    async startHomeServiceFromToolbar(uri) {
        try {
            ...
            if (uri && uri.scheme != 'webview-panel') {
                ...
                const savedServiceDirectory = ServiceStateManager_1.ServiceStateManager.getSelectedServiceDirectory();
                if (savedServiceDirectory && fs.existsSync(savedServiceDirectory)) {
                    ...
                    if (fs.existsSync(markerFilePath)) {
                        ...
                        try {
                            await ServiceStateManager_1.ServiceStateManager.clearSelectedServiceDirectory();
                        }
                        catch (clearError) {
