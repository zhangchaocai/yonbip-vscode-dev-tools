...
` +
            ...
            StatisticsService_1.StatisticsService.incrementCount(StatisticsService_1.StatisticsService.PATCH_EXPORT_COUNT);
...
<body>
    ...
    async _buildReplacementContent(files, patchInfo, archive, basePath) {
        ...
        const createDirectoryWithTimestamp = (directoryPath) => {
...
`;
    ...
    async _validateJavaFilesHaveCompiledFiles(selectedPath, selectedPaths, basePath) {
        const fs = require('fs');
        const path = require('path');
        try {
            let filesToCheck = [];
    ...
    async _collectJavaFiles(basePath) {
        const fs = require('fs');
        const path = require('path');
        const javaFiles = [];
        try {
    ...
    async _forceRefreshWorkspace(showMessages = true) {
        try {
