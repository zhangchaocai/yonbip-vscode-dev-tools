...
const HomeVersionUtils_1 = require("../../utils/HomeVersionUtils");
...
class ProjectCommands {
    ...
    async createProject(projectPath) {
        ...
        const confirm = await this.showCustomConfirmationDialog('创建项目', isMultiModuleProject || isInMultiModuleRoot ?
        ...
        try {
            ...
            try {
                const iconUpdated = await IconThemeUpdater_1.IconThemeUpdater.addModuleToIconTheme(folderName);
                if (iconUpdated) {
    ...
    async createMultiModuleProject(projectPath) {
        ...
        const confirm = await this.showCustomConfirmationDialog('创建多模块项目', `将在工作区下创建多模块项目文件夹：${folderName}\n\n完整路径：${selectedPath}\n这将创建build/classes目录。是否继续？`);
        if (!confirm) {
            return;
        }
        try {
    ...
    async createComponent(componentPath) {
        ...
        const confirm = await this.showCustomConfirmationDialog('创建业务组件', `将在以下目录创建业务组件：${targetPath}\n\n是否继续？`);
        if (!confirm) {
            return;
        }
        try {
...
<module name="${dirName}">
    ...
    async showCustomConfirmationDialog(title, message) {
    ...
    getConfirmationDialogHtml(title, message) {
