...
def _load_dotenv(env_path: Optional[Path] = None) -> None:
    ...
    if env_path is None:
        # In common package, we need to adjust the path resolution
        # When called from skill scripts, _load_dotenv is called after adding
        # common to sys.path, but we need to find the skill root
        # Leave it to caller to handle path
        from inspect import getsourcefile
        current_file = getsourcefile(lambda: None)
        if current_file:
            # Go up: common/iuap_common/secure_config.py → common/iuap_common → common → skills
