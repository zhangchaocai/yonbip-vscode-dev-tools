...
def validate_api_config(config: Dict[str, Any]) -> List[str]:
    ...
    # 如果已经提供了 queryUri/query_uri，则不需要 allbillname
    has_query_uri = bool(
        req.get("queryUri") or req.get("query_uri") or ""
    )
    if has_query_uri:
        # 有 queryUri 直查，不需要 allbillname
        required_request_fields = []
    else:
        required_request_fields = ["allbillname"]

    return validate_required_fields(
        config,
        {
            "api": ["base_url", "app_key", "app_secret"],
            "request": required_request_fields,
        },
    )
