...
Features:
    ...
    - 环境变量支持（避免硬编码密钥）
    - 详细错误信息
    - HTTP 连接复用（requests.Session 全局单例）
    - 按 API 端点分层限流
...
def _get_session() -> requests.Session:
    """获取全局 HTTP Session（单例，复用 TCP 连接）"""
...
def _classify_path_to_rate_key(path: str) -> str:
    """根据请求路径分类到不同的限流器"""
    if "queryByUri" in path:
        return "queryByUri"
    elif "searchByName" in path:
        return "searchByName"
    elif "getEntityListByBOId" in path:
        return "getEntityListByBOId"
    elif "getEntityInfoByBOIdAndEntityId" in path:
        return "getEntityInfo"
    elif "getAccessToken" in path:
        return "token"
    else:
        return "default"


def _get_rate_limiter(cfg: Dict[str, Any], path: str) -> RateLimiter:
    """获取限流器，按 API 端点分层配置"""
    global _rate_limiters
    rate_key = _classify_path_to_rate_key(path)

    with _rate_limiters_lock:
        if rate_key in _rate_limiters:
            return _rate_limiters[rate_key]

        # 从配置读取分层参数，回退到全局默认
        rate_limit_cfg = cfg.get("rate_limit", {})
        # 分层参数名：rps_{key} / burst_{key}
        rps_key = f"rps_{rate_key}"
        burst_key = f"burst_{rate_key}"

        if rps_key in rate_limit_cfg:
            rate = float(rate_limit_cfg[rps_key])
        else:
            rate = float(rate_limit_cfg.get("requests_per_second", 10.0))

        if burst_key in rate_limit_cfg:
            burst = float(rate_limit_cfg[burst_key])
        else:
            burst = float(rate_limit_cfg.get("burst_capacity", rate * 2))

        limiter = RateLimiter(rate=rate, capacity=burst)
        _rate_limiters[rate_key] = limiter
        logger.debug(
            f"Created rate limiter for '{rate_key}': rate={rate:.1f}/s, burst={burst:.1f}"
        )
        return limiter
...
def _request_get_java_style(
    ...
    *,
    ...
    full = f"{url}?{qs}"
    return session.get(full, timeout=timeout, verify=verify)
...
def _fetch_token_from_server(cfg: Dict[str, Any]) -> Tuple[str, float]:
    ...
    rate_limiter = _get_rate_limiter(cfg, path)
...
def http_post_json(
    ...
    *,
    ...
    # 使用全局 Session 复用 TCP 连接
    session = _get_session()
    r = session.post(url, params=params, json=body, timeout=timeout, verify=verify)
