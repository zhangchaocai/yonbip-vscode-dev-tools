...
def _skill_dir() -> Path:
    return _get_scripts_dir().parent
...
def _get_db_connection(cfg: dict, driver: str):
    ...
    try:
        if driver == "mysql":
            ...
            password = db.get("password", "")
            database = db.get("database", "")
            # dmPython支持多种连接方式，优先使用keyword参数
            try:
                ...
                # 备选：尝试简化的位置参数方式
                conn = dmPython.connect(user, password, host, port, database)
...
def _replace_param_vars(sql_text: str) -> tuple:
    ...
    for param_name in matches:
        ...
        if replacement is None:
            # 智能推断参数类型
            if "期间" in param_name or "日期" in param_name or "期间" in param_name:
                replacement = "'2024-01'"
