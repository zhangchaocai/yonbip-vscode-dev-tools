...
def _split_billnames_raw(cell_value: str) -> Tuple[List[str], List[str]]:
    ...
    def _process_part(part: str) -> None:
        ...
        if not part:
            return
        if part in _BILLNAME_SUFFIX_WORDS:
            return
        if part not in seen:
            seen.add(part)
            results.append(part)
...
def _find_core_billname_col(headers: List[str]) -> int:
    """
    找到"核心业务对象"列的索引。
    搜索所有列头，匹配 _CORE_BILLNAME_HEADERS 中的关键词（精确或包含）。
    """
...
def _find_bill_objects_section(rows: List[List[str]]) -> Tuple[int, List[Dict[str, Any]]]:
    ...
    for row_idx, row in enumerate(rows):
        ...
        if start_idx >= 0 and row_idx >= start_idx:
            # 第一列：业务对象名称
            bill_name = row[0].strip() if row else ""
            ...
            if bill_name:
                ...
                if not any(pat in bill_name for pat in exclude_patterns):
                    # 排除日期格式
                    if not (len(bill_name) == 10 and "/" in bill_name):
                        # 排除纯数字编号
                        if not bill_name.isdigit():
                            # 检查是否是常见的业务对象后缀
                            is_business_object = (
                                any(bill_name.endswith(suf) for suf in bill_suffixes) or
                                any(suf in bill_name for suf in ["订单", "合同", "发货", "出库", "发票", "应收", "应付", "物料", "客户", "供应商", "组织"])
                            )
                            ...
                            if is_business_object:
                                ...
                                bill_objects.append({
                                    "billName": bill_name,
                                    "relation": relation if relation and "通过" in relation else ""
                                })
    ...
    for row_idx, row in enumerate(data_rows):
        ...
        if billname_col >= 0 and billname_col < len(row):
            ...
            if cell:
                ...
                row_billnames.update(bill_part)
