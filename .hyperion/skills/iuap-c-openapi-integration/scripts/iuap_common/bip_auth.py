...
def get_access_token(cfg: Dict[str, Any], *, force_refresh: bool = False) -> str:
    ...
    流程:
        1. 检查缓存是否有效
        2. 缓存无效或 force_refresh 时获取刷新锁
        3. 双检锁：其他线程可能已刷新，再次检查缓存
        4. 从服务器获取并更新缓存
    ...
    # 缓存未命中或强制刷新：获取刷新锁，防止多个线程同时刷新
    refresh_lock = _get_refresh_lock(key)
    with refresh_lock:
        # 双检锁：其他线程可能已经刷新了缓存
        if not force_refresh:
            with _lock:
                ...
                if ent is not None:
                    token, expires_at = ent
                    if now < expires_at:
                        logger.debug("双检命中缓存的 token (剩余 %.0f 秒)", expires_at - now)
                        return token
